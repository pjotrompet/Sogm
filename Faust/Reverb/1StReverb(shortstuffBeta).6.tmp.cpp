...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void instanceInit(int samplingFreq) {
		...
		for (int i=0; i<262144; i++) iRec1[i] = 0;
		...
		for (int i=0; i<2048; i++) fRec9[i] = 0;
		for (int i=0; i<4096; i++) fRec10[i] = 0;
		for (int i=0; i<512; i++) fRec11[i] = 0;
		for (int i=0; i<4096; i++) fRec12[i] = 0;
		fslider3 = 5.0f;
		fslider4 = 1.0f;
	}
	...
	virtual void buildUserInterface(UI* interface) {
		interface->openVerticalBox("1StReverb(shortstuffBeta).6.tmp");
		...
		interface->addHorizontalSlider("dry", &fslider4, 1.0f, 0.0f, 1e+02f, 0.1f);
		interface->addHorizontalSlider("grainfreq", &fslider2, 25.0f, 25.0f, 1e+03f, 0.1f);
		interface->addHorizontalSlider("wet", &fslider3, 5.0f, 0.0f, 1e+02f, 0.1f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		float 	fSlow17 = fcheckbox0;
		float 	fSlow18 = (1 - fSlow17);
		int 	iSlow19 = int((iConst0 / fslider2));
		int 	iSlow20 = (7 * iSlow19);
		float 	fSlow21 = (0.001f * min(100, max(0, fslider3)));
		float 	fSlow22 = min(100, max(0, fslider4));
		...
		for (int i=0; i<count; i++) {
			...
			iRec2[0] = faustpower<iSlow19>((7 * iRec2[1]));
			iRec3[0] = (7 * (iSlow19 + iRec3[1]));
			iRec1[IOTA&262143] = (1 + iRec1[(IOTA-int((1 + int(abs(int(min(192000, max(1, (int(((iSlow20 * (iRec3[0] * faustpower<3>(iRec2[0]))) % iSlow19)) % 192000)))))))))&262143]);
			float fTemp28 = ((fSlow17 * ((iRec1[(IOTA-0)&262143] - 1) * (fTemp27 + fTemp26))) + (fSlow18 * fVec14[(IOTA-iSlow16)&65535]));
			fRec0[IOTA&511] = (fTemp28 + (fSlow0 * fRec0[(IOTA-462)&511]));
			float fTemp29 = fRec0[(IOTA-0)&511];
			fRec4[IOTA&4095] = (fTemp28 + (fSlow0 * fRec4[(IOTA-2601)&4095]));
			fRec5[IOTA&4095] = (fTemp28 + (fSlow0 * fRec5[(IOTA-2914)&4095]));
			fRec6[IOTA&2047] = (fTemp28 + (fSlow0 * fRec6[(IOTA-1724)&2047]));
			fRec7[IOTA&2047] = (fTemp28 + (fSlow0 * fRec7[(IOTA-1026)&2047]));
			fRec8[IOTA&511] = (fTemp28 + (fSlow0 * fRec8[(IOTA-302)&511]));
			float fTemp30 = ((((fRec8[(IOTA-0)&511] + fRec7[(IOTA-0)&2047]) + fRec6[(IOTA-0)&2047]) + fRec5[(IOTA-0)&4095]) + fRec4[(IOTA-0)&4095]);
			fRec9[IOTA&2047] = (fTemp28 + (fSlow0 * fRec9[(IOTA-1318)&2047]));
			float fTemp31 = fRec9[(IOTA-0)&2047];
			fRec10[IOTA&4095] = (fTemp28 + (fSlow0 * fRec10[(IOTA-2318)&4095]));
			float fTemp32 = fRec10[(IOTA-0)&4095];
			fRec11[IOTA&511] = (fTemp28 + (fSlow0 * fRec11[(IOTA-507)&511]));
			float fTemp33 = fRec11[(IOTA-0)&511];
			fRec12[IOTA&4095] = (fTemp28 + (fSlow0 * fRec12[(IOTA-3132)&4095]));
			float fTemp34 = fRec12[(IOTA-0)&4095];
			float fTemp35 = (fSlow22 * fVec0[0]);
			output0[i] = (FAUSTFLOAT)(fTemp35 + (fSlow21 * (fTemp34 + (fTemp33 + (fTemp32 + (fTemp31 + (fTemp30 + fTemp29)))))));
			output1[i] = (FAUSTFLOAT)(fTemp35 + (fSlow21 * (0 - (((((fTemp29 + fTemp31) + fTemp32) + fTemp33) + fTemp34) - fTemp30))));
			// post processing
			iRec3[1] = iRec3[0];
			iRec2[1] = iRec2[0];
			IOTA = IOTA+1;
